...
#priority queue that doesn't add duplicate nodes
...
class AlgorithmPathfinder(object):
    ...
    def __init__(
        ...
        self._algorithms = {0: 'dijkstra', 1: 'astar'}
        self._path_found = False

        # Create window
        self._clock = pygame.time.Clock()
        self._fps = 60
        self._surface = create_example_window('Algorithm Pathfinder', (900, 650))
    ...
    def _setup_menu(self) -> None:
        ...
        menu_about.add.label("Algorithm \nPathfinder", font_name=pygame_menu.font.FONT_OPEN_SANS_BOLD, font_size=25,
                             margin=(0, 5))
        menu_about.add.vertical_margin(10)
        text = 'To move the start/end points, left click and drag.\n' \
    ...
    def _better_prim(self, mazearray: Optional[_MazeType] = None, start_point: Optional[_Point2] = None) -> _MazeType:
        """
        Randomized Prim's algorithm for creating random mazes. Alt prim.
    ...
    def _dijkstra(
        ...
        """
        Option to switch to A* by adding a heuristic of expected distance to end node.
...
def main(test: bool = False) -> 'AlgorithmPathinder':
    """
    Main function.
    :param test: Indicate function is being tested
    :return: App
    """
    app = AlgorithmPathfinder()
    app.mainloop(test)
    return app
